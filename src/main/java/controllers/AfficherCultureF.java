...
public class AfficherCultureF implements Initializable, FrontboardController.UserAwareController {
    ...
    public void setUser(User user) {
        ...
        loadCultures();  // Charger les cultures de cet utilisateur
    }
    ...
    private VBox createCultureCard(Culture culture) {
        ...
        Label date = new Label("Plantation: " + culture.getDatePlantation().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
    ...
    private void loadCultures() {
        try {
            ...
            if (currentUser != null) {
                cultureService.setUser(currentUser); // important

                if (currentUser.getRole() == Role.ADMIN) {
                    list = cultureService.select(); // admin voit tout
                } else {
                    list = cultureService.getCulturesByUser();
                }
            } else {
                // Cas très rare si setUser n’est jamais appelé
                list = cultureService.select(); // admin par défaut
            }
    ...
    private void showPredictionAlert(String message) {
        ...
        alert.setHeaderText("Analyse terminée");
        alert.setContentText("Résultat prédit : \n" + message);
        alert.showAndWait();
    }
